# Please note, this is a slimmed down version of the original common.py and it only support pytorch
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            layer_name = 'model.27'  # Adjust this to the layer you want to extract features from
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            # Predict
            self.features = [] # Reset the features array
            y = self.model(im, augment=augment, visualize=visualize) if augment or visualize else self.model(im)
        else:
            raise NotImplementedError(f"ERROR: only pytorch is supported")

        if isinstance(y, (list, tuple)):
            return self.from_numpy(y[0]) if len(y) == 1 else [self.from_numpy(x) for x in y], self.from_numpy(self.features[0])
        else:
            return self.from_numpy(y), self.from_numpy(self.features[0])
